...
def summarize_performance(step, g_model, gan_model, latent_dim, n_cat, n_samples=100):
    ...
    # save plot to file
    filename1 = 'output_files/sgd/generated_plot_%04d.png' % (step + 1)
    pyplot.savefig(filename1)
    pyplot.close()


# create a line plot of loss for the gan and save to file
def plot_history(d_hist, g_hist, q_hist):
    # plot loss
    pyplot.subplot(2, 1, 1)
    pyplot.ylim([0, 1.1])
    pyplot.plot(d_hist, label='D-loss')
    pyplot.plot(g_hist, label='G-loss')
    pyplot.plot(q_hist, label='Q-loss')
    pyplot.legend()
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_cat, n_epochs=100, n_batch=64):
    d_hist, g_hist, q_hist = list(), list(), list()
    ...
    for i in range(14000):
        ...
        print('>%d, d[%.3f], g[%.3f] q[%.3f]' % (i + 1, d_loss, g_1, g_2))
        if (i + 1) % 460 == 0:
            losses.write('D Loss = ' + str(d_loss) + ' : G Loss = ' + str(g_1) + '\n')
        if (i + 1) % 1400 == 0:
            summarize_performance(i, g_model, gan_model, latent_dim, n_cat)
    plot_history(d_hist, g_hist, q_hist)
    summarize_performance(i, g_model, gan_model, latent_dim, n_cat)
